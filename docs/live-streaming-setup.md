...
playback URL is:
```
https://stream.capitony.live/hls/{stream_key}.m3u8
```

## 7. Streaming from the captain's phone

Install **Larix Broadcaster** (free, iOS + Android). In its settings, add
a connection:
- **URL**: `rtmp://stream.capitony.live/live`
- **Stream key**: exactly the `stream_key` value shown on the captain's
  dashboard when they click "Go Live" (already generated by the app —
